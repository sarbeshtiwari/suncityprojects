  ...
  const fetchEnquiries = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const cutoffDate = new Date('2026-02-04');

        setEnquiries(
          data.forms.filter(
            (item: { pageName: string; createdAt: string }) => {
              const createdAtDate = new Date(item.createdAt);
              const hasMonarch = item.pageName.includes('monarch');
              const isBefore4Feb = createdAtDate < cutoffDate;

              return isBefore4Feb || !hasMonarch;
            }
          )
        );
